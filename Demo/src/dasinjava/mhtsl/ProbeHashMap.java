...
public class ProbeHashMap<K, V> extends AbstractHashMap<K, V> {
	private MapEntry<K,V>[] table;			// a fixed array of entries (all initally null)
	...
	private int findSlot(int h, K k) {
		...
		do {
			if (isAvailable(j))	{			// may be either empty nor dufunct
	...
	protected V bucketPut(int h, K k, V v) {
		...
		table[-(j+1)] = new MapEntry<>(k,v);	// conver to proper index
		n++;
		return null;
	}
