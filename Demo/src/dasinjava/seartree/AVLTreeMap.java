package dasinjava.seartree;

import java.util.Comparator;

import dasinjava.list.Position;
import dasinjava.priqueue.Entry;
import dasinjava.seartree.TreeMap;
...
public class AVLTreeMap<K,V> extends TreeMap<K, V> {
	...
	/** Returns whether a position has balanced factor between -1 and 1 inclusive. */
	protected boolean isBalanced(Position<Entry<K,V>> p) {
		return Math.abs(height(left(p)) - height(right(p))) <= 1;
	}
	/** Returns a child of p with height no smaller than that of the the other child. */
	 ...
	 * path upward from p, perfomring a trinode restructuring when imbalance is found.
